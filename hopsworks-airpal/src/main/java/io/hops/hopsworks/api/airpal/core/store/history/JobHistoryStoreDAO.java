...
import io.hops.hopsworks.api.airpal.sql.Util;
import io.hops.hopsworks.api.airpal.sql.beans.JobTableOutputJoinRow;
import io.hops.hopsworks.api.airpal.sql.beans.JobTableRow;
import io.hops.hopsworks.api.airpal.sql.beans.TableRow;
import io.hops.hopsworks.api.airpal.sql.dao.JobDAO;
import io.hops.hopsworks.api.airpal.sql.dao.JobOutputDAO;
import io.hops.hopsworks.api.airpal.sql.dao.JobTableDAO;
import io.hops.hopsworks.api.airpal.sql.dao.TableDAO;
  ...
  private List<Job> getJobs(long limit, int dayInterval, String outerWhereClauseArg, String innerWhereClauseArg) {
    ...
    try (Handle handle = dbi.open()) {
      Query<Map<String, Object>> query = handle.createQuery(
          "SELECT " + "j.id AS id, " + "j.query AS query, " + "j.user AS user, " + "j.uuid AS uuid, "
          + "j.queryStats as queryStats, " + "j.state AS state, " + "j.columns AS columns, "
          + "j.query_finished AS queryFinished, " + "j.query_started AS queryStarted, " + "j.error AS error, "
          + "t.connector_id AS connectorId, " + "t.schema_ AS \"schema\", " + "t.table_ AS \"table\", " + "t.columns, "
          + "jo.type, " + "jo.description, " + "jo.location " + "FROM (SELECT * FROM jobs " + "WHERE " + Util.
          getQueryFinishedCondition(dbType) + " " + "AND " + innerWhereClause + " "
          + "ORDER BY query_finished DESC LIMIT :limit) j " + "LEFT OUTER JOIN job_tables jt ON j.id = jt.job_id "
          + "LEFT OUTER JOIN tables t ON jt.table_id = t.id " + "LEFT OUTER JOIN job_outputs jo ON j.id = jo.job_id "
          + "WHERE " + outerWhereClause + " " + "ORDER BY query_finished DESC")
          .bind("limit", limit)
