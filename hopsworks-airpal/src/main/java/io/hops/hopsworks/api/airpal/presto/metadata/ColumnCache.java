...
import io.hops.hopsworks.api.airpal.core.BackgroundCacheLoader;
import io.hops.hopsworks.api.airpal.core.execution.QueryClient;
import io.hops.hopsworks.api.airpal.presto.QueryRunner;
import io.hops.hopsworks.api.airpal.presto.Util;
import io.hops.hopsworks.api.airpal.presto.hive.HiveColumn;
...
public class ColumnCache {
  ...
  public ColumnCache(final QueryRunnerFactory queryRunnerFactory,
    ...
    BackgroundCacheLoader<String, List<HiveColumn>> columnLoader = new BackgroundCacheLoader<String, List<HiveColumn>>(
        listeningExecutor) {
      @Override
    ...
    BackgroundCacheLoader<String, List<HivePartition>> partitionLoader
        = new BackgroundCacheLoader<String, List<HivePartition>>(listeningExecutor)
          {
            @Override
  ...
  private List<HivePartition> queryPartitions(String query) {
    ...
    QueryClient queryClient = new QueryClient(queryRunner, org.joda.time.Duration.millis(60 * 1000),
        query);
  ...
  private List<HiveColumn> queryColumns(String query) {
    ...
    QueryRunner queryRunner = queryRunnerFactory.create();
    QueryClient queryClient = new QueryClient(queryRunner, org.joda.time.Duration.millis(60), query);
      ...
      queryClient.executeWith(new Function<StatementClient, Void>() {
        ...
        public Void apply(StatementClient client) {
          ...
          if (results.getData() != null) {
            for (List<Object> row : results.getData()) {
              Column column = new Column((String) row.get(0), (String) row.get(1), new ClientTypeSignature(
                  TypeSignature.parseTypeSignature((String) row.get(1))));
