package io.hops.hopsworks.api.airpal.core.store.files;
...
public class ExpiringFileStore {
  ...
  private final LoadingCache<String, FileWithMetadata> fileWithMetadataCache;
  ...
  public ExpiringFileStore(DataSize maxStorageSize) {
    ...
    this.fileWithMetadataCache = CacheBuilder.newBuilder().maximumWeight(maxWeightInBytes).weigher((String key,
        FileWithMetadata fileWithMetadata) ->
        (int) Math.round(fileWithMetadata.getSize().getValue(DataSize.Unit.BYTE))).removalListener(
            new RemovalListener<String, FileWithMetadata>()
            {
              @Override
            ...
            }).build(new CacheLoader<String, FileWithMetadata>()
            {
              ...
              public FileWithMetadata load(String key) throws Exception 
              {
                ...
                if (file.exists())
                {
                  return new FileWithMetadata(file, new DataSize(file.length(), 
                      DataSize.Unit.BYTE), DateTime.now());
                }
  ...
  public void addFile(String key, File file)
    ...
    fileWithMetadataCache.put(key,
        new FileWithMetadata(file, new DataSize(fileSize, DataSize.Unit.BYTE), DateTime.now()));
  }
