package io.hops.hopsworks.airpal.core.execution;

import io.hops.hopsworks.airpal.api.Job;
import io.hops.hopsworks.airpal.api.JobState;
import io.hops.hopsworks.airpal.api.event.JobUpdateEvent;
import io.hops.hopsworks.airpal.api.output.InvalidQueryException;
import io.hops.hopsworks.airpal.api.output.builders.FileTooLargeException;
...
import io.hops.hopsworks.airpal.api.output.persistors.Persistor;
import io.hops.hopsworks.airpal.api.output.persistors.PersistorFactory;
import io.hops.hopsworks.airpal.core.execution.QueryClient.QueryTimeOutException;
...
import static io.hops.hopsworks.airpal.core.execution.ExecutionClient.ExecutionFailureException;
import static io.hops.hopsworks.airpal.presto.QueryInfoClient.BasicQueryInfo;
...
public class Execution implements Callable<Job> {
  ...
  private Job doExecute()
    ...
    try {
      ...
      job.setError(new QueryError(e.getMessage(), null, -1, null, null, new ErrorLocation(e.getLineNumber(), e.
    ...
    try {
      ...
      throw new ExecutionFailureException(job,
          format("Query exceeded maximum execution time of %s minutes", Duration.millis(e.getElapsedMs()).
              getStandardMinutes()),
          e);
    }
